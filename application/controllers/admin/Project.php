...
class Project extends MY_Controller
{
    ...
    public $pageTitle 	= '项目管理';
    public $modelName   = 'project_model';
    public $searchFile  = 'admin/project_search.html';#搜索文件
    public $pageTips    = '项目管理';
    public $checkCol    = 0;
    ...
    {
    	$data['pageTitle'] = '新增项目';
    	$this->display('admin/project_add.html',$data);
    }
    ...
    public function tab_edit()
    {
    	...
    	$project = $this->project_model->getConditionData('*','id='.(int)$id);
    	if (!isset($project[0]['id'])) {
    		echo "数据不存在";
    		exit();
    	}
    	$data['project'] = $project[0];
    	$this->display('admin/project_edit.html',$data);
    }
    ...
    public function detail()
    {
    	...
    	$main=[];
    	if($project[0]['main_conn']){
    	    $main = $this->connection_model->getConditionData("username,email,phone,company",'id = '.$project[0]['main_conn'],1);
    	}
    	//其他联系人处理
    	$other = [];
    	if ($project[0]['other_conn']) {
    	    $other = $this->connection_model->getConditionData("username,email,phone,company",'id IN('.$project[0]['other_conn'].')',1);
    	    if($other){
    	...
    	$data['project'] = $project[0];
    	$this->display('admin/project_detail.html',$data);
    }
    ...
    public function save()
    {
        ...
        if(isset($data['id']) && $data['id']>0){
	        ...
	        $data['edit_time']   = time();
	        $res = $this->project_model->editData($data,'id="'.$data['id'].'"');
        ...
        //查找标题是否存在
        $exist = $this->project_model->getConditionData("id,name","name='".addslashes($data['name'])."'");
        if(isset($exist[0]['name'])){
        	$this->ajaxReturn('',300,'项目名称重复');
        }
        $data['add_time'] = $data['edit_time'] = time();
        $res = $this->project_model->addData($data,1,1);
        $res ? $this->ajaxReturn($res) : $this->ajaxReturn($res,300,'保存失败');
    ...
    public function change_verify()
    {
    	...
    	$data['edit_time']   = time();
        $report = $this->project_model->getConditionData('*','id='.$data['id'])[0];
        $res = $this->project_model->editData($data,'id="'.$data['id'].'"');
        if($data['status'] == 3 && $report['status'] != 3){
            ...
            $post_data['uid'] = $uid;
            $post_data['type'] = '项目合作';
            $post_data['order_no'] = $report['project_no'];
    ...
    public function delete()
    {
    	...
    	$data['status'] = 0;
    	$res = $this->project_model->editData($data,'id="'.(int)$id.'"');
