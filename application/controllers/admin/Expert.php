...
class Expert extends MY_Controller
{
    ...
    public $pageTitle 	= '专家管理';
    public $modelName   = 'expert_model';
    public $searchFile  = 'admin/expert_search.html';#搜索文件
    public $pageTips    = '专家管理';
    public $checkCol    = 0;

    public $role = [];

    public $admin_info;
    public static $status_text = ['删除',"审核中","驳回","正常"];
    ...
    {
    	$data['pageTitle'] = '新增专家';
    	$this->display('admin/expert_add.html',$data);
    }
    ...
    public function tab_edit()
    {
    	...
    	$expert = $this->expert_model->getConditionData('*','id='.(int)$id);
    	if (!isset($expert[0]['id'])) {
    		echo "数据不存在";
    		exit();
    	}
    	$data['expert'] = $expert[0];
    	$this->display('admin/expert_edit.html',$data);
    }
    ...
    public function detail()
    {
    	...
    	$main=[];
    	if($expert[0]['main_conn']){
    	    $main = $this->connection_model->getConditionData("username,email,phone,company",'id = '.$expert[0]['main_conn'],1);
    	}
    	//其他联系人处理
    	$other = [];
    	if ($expert[0]['other_conn']) {
    	    $other = $this->connection_model->getConditionData("username,email,phone,company",'id IN('.$expert[0]['other_conn'].')',1);
    	    if($other){
    	...
    	$data['expert'] = $expert[0];
    	$this->display('admin/expert_detail.html',$data);
    }
    ...
    public function save()
    {
        ...
        if(isset($data['id']) && $data['id']>0){
	        ...
	        $data['edit_time']   = time();
	        $res = $this->expert_model->editData($data,'id="'.$data['id'].'"');
        ...
        //查找标题是否存在
        $exist = $this->expert_model->getConditionData("id,name","name='".addslashes($data['name'])."'");
        if(isset($exist[0]['name'])){
        	$this->ajaxReturn('',300,'专家名称重复');
        }
        $data['add_time'] = $data['edit_time'] = time();
        $res = $this->expert_model->addData($data,1,1);
        $res ? $this->ajaxReturn($res) : $this->ajaxReturn($res,300,'保存失败');
    ...
    public function change_verify()
    {
    	...
    	$data['edit_time']   = time();
        $report = $this->expert_model->getConditionData('*','id='.$data['id'])[0];
        $res = $this->expert_model->editData($data,'id="'.$data['id'].'"');
        if($data['status'] == 3 && $report['status'] != 3){
            ...
            $post_data['uid'] = $uid;
            $post_data['type'] = '申请专家';
            $post_data['order_no'] = $report['expert_no'];
    ...
    //     }
    //     //首先全部不置顶
    //     $this->expert_model->editData(['is_top'=>0],'id>0');

    //     $data['id'] = $id;
    //     $data['is_top'] = 1;
    //     $data['edit_time']   = time();
    //     $res = $this->expert_model->editData($data,'id="'.$id.'"');
    ...
    public function delete()
    {
    	...
    	$data['status'] = 0;
    	$res = $this->expert_model->editData($data,'id="'.(int)$id.'"');
