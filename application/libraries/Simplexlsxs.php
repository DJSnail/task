<?php
/*
	Simplexlsxs php class v0.6.5
	MS Excel 2007 workbooks reader

	Example 1: 
	$xlsx = new Simplexlsxs('book.xlsx');
	print_r( $xlsx->rows() );
	...
	Example 6:
	$xslx = new Simplexlsxs( file_get_contents('http://www.example.com/example.xlsx'), true);
...
class Simplexlsxs
{
	...
	function __construct( $filename, $is_data = false ) {
		//按照CI框架的规则$filename是个数组
		$this->_unzip( $filename[0], $is_data );
		$this->_parse();
	}
	function sheets() {
		return $this->sheets;
...
//		$this->styles = array();
		...
		if ( $relations = $this->getEntryXML("_rels/.rels" ) ) {
			...
			foreach ($relations->Relationship as $rel) {
				...
				if ($rel["Type"] == Simplexlsxs::SCHEMA_REL_OFFICEDOCUMENT) {
...
//							echo 'workbook relations<br />';
							...
							foreach ($workbookRelations->Relationship as $workbookRelation) {
								...
								if ($workbookRelation['Type'] == Simplexlsxs::SCHEMA_REL_WORKSHEET) { // Sheets
...
//										echo '<pre>'.htmlspecialchars( print_r( $sheet, true ) ).'</pre>';
								...
								} else if ($workbookRelation['Type'] == Simplexlsxs::SCHEMA_REL_SHAREDSTRINGS) {
...
//									echo 'sharedstrings<br />';
								...
								} else if ($workbookRelation['Type'] == Simplexlsxs::SCHEMA_REL_STYLES) {
